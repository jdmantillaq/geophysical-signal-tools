...
### `eof_analysis.py`
- **`compute_eof(data)`**: Computes EOFs from a 3D array or `xarray.DataArray` shaped as `(time, lat, lon)`.
...
- **`project_data(data, vec_prop)`**: Projects the 3D data onto the provided eigenvectors to obtain mode time series/fields.
	- **Input**: 3D data `(time, lat, lon)`, `vec_prop` with shape `(time, time)`
	- **Returns**: Projected data with the same shape as the input.

Notes:
- The routines assume you provide data already preprocessed as desired (e.g., anomalies/detrended). No demeaning is applied inside `compute_eof`.
- NaN handling: grid cells that are NaN in the first time step are excluded from the analysis and restored on output.

### `spectral_analysis.py`
- **`fourier_bandpass_filter(series, low_period, high_period)`**: Simple Fourier band‑pass using period bounds.
...
- **`compute_fourier_spectrum(time_series)`**: Computes normalized power spectrum (% variance) via FFT.
	- **Returns**: `(periods, percent_variance)` arrays.
- **`plot_fourier_spectra(serie)`**: Convenience plot of the Fourier spectrum of a 1D series.
...
from eof_analysis import compute_eof, project_data
from spectral_analysis import fourier_bandpass_filter, compute_fourier_spectrum, plot_fourier_spectra
...
val_prop, vec_prop, eof, var_exp = compute_eof(data)

# Project back using eigenvectors (shape must be [time, time])
proj = project_data(data, vec_prop)
```
...
from spectral_analysis import (
		fourier_bandpass_filter,
		compute_fourier_spectrum,
		plot_fourier_spectra,
)
...
filtered = fourier_bandpass_filter(series, low_period=10, high_period=40)

# Spectrum (% variance by period)
periods, pct_var = compute_fourier_spectrum(series)

# Quick plot
plot_fourier_spectra(series)
```
